...
mkdir -p "${BATOCERA_BINARIES_DIR}/build-uboot-lemo-xiaoc"     || exit 1
cp "${BOARD_DIR}/build-uboot.sh"          "${BATOCERA_BINARIES_DIR}/build-uboot-lemo-xiaoc/" || exit 1
cd "${BATOCERA_BINARIES_DIR}/build-uboot-lemo-xiaoc/" && ./build-uboot.sh "${HOST_DIR}" "${BOARD_DIR}" "${BINARIES_DIR}" || exit 1
...
cp "${BINARIES_DIR}/meson-g12b-a311d-cainiao-lemo-xiaoc.dtb"  "${BATOCERA_BINARIES_DIR}/boot/boot/"          || exit 1
