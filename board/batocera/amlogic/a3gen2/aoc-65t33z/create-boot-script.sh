...
mkdir -p "${BATOCERA_BINARIES_DIR}/build-uboot-aoc-65t33z"     || exit 1
cp "${BOARD_DIR}/build-uboot.sh"          "${BATOCERA_BINARIES_DIR}/build-uboot-aoc-65t33z/" || exit 1
cd "${BATOCERA_BINARIES_DIR}/build-uboot-aoc-65t33z/" && ./build-uboot.sh "${HOST_DIR}" "${BOARD_DIR}" "${BINARIES_DIR}" || exit 1
...
cp "${BINARIES_DIR}/t7-a311d2-65t33z-4g.dtb"  "${BATOCERA_BINARIES_DIR}/boot/boot/amlogic/"          || exit 1
